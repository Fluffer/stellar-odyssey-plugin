...
function installGalaxyInfo(ctx) {
  ...
  // Fuel efficiency comes from the map component, which only exists on the
  // galaxy page. Remember the last value so jumps observed on other pages
  // are costed correctly instead of at the full 10 ly per cell.
  var LS_EFF = ctx.lsPrefix + "fuelEff";
  var effCache = null;
  function fuelEff() {
    var p = props();
    if (typeof p.fuelEfficiencyBonus === "number") {
      if (effCache !== p.fuelEfficiencyBonus) { effCache = p.fuelEfficiencyBonus; try { localStorage.setItem(LS_EFF, String(effCache)); } catch (e) {} }
      return effCache;
    }
    if (effCache === null) { try { var v = localStorage.getItem(LS_EFF); effCache = v === null ? null : Number(v); } catch (e) { effCache = null; } }
    return effCache;
  }
  function fuelEffKnown() { return typeof fuelEff() === "number" && isFinite(fuelEff()); }
  function fuelCost(ly) { return ly * (1 - num(fuelEff()) / 100); }
  ...
  function newSession() { return { startedAt: Date.now(), jumps: 0, newSystems: 0, dust: 0, xp: 0, fuelSpent: 0, ly: 0, moves: 0, trail: [], lastSig: null, voy: { ids: [], jumps: 0, xp: 0, dust: 0, net: 0 } }; }
  ...
  function trackPosition(ex) {
    ...
    // Only moves whose fuel cost is known feed the average; a move seen
    // before the efficiency was ever read would be charged the full 10 ly.
    if (moved && fuelEffKnown()) {
  ...
  // --- voyager expedition ------------------------------------------------
  // Each waypoint carries its cosmic dust reward (net) and the taxed part;
  // the game's Voyager page prints "Exploring XP gain" as
  // floor((sum reward + sum taxed) / 20), so XP per waypoint is gross dust / 20.
  function voyagerSummary() {
    var vs = getStoreById("Voyager");
    var v = vs && vs.$state;
    if (!v || !Array.isArray(v.queue) || !v.queue.length) return null;
    var out = { total: v.queue.length, reached: 0, pending: 0, xp: 0, dust: 0, net: 0, eta: 0, items: [] };
    for (var i = 0; i < v.queue.length; i++) {
      var q = v.queue[i];
      if (!q) continue;
      var net = num(q.cosmic_dust_reward), gross = net + num(q.cosmic_dust_reward_taxed);
      var reached = q.status === "reached";
      var xp = reached ? Math.floor(gross / 20) : 0;
      if (reached) { out.reached++; out.xp += xp; out.dust += gross; out.net += net; } else out.pending++;
      if (num(q.eta) > out.eta) out.eta = num(q.eta);
      out.items.push({ id: q._id || (q.destination_x + "," + q.destination_y + "," + q.start), reached: reached, xp: xp, gross: gross, net: net, x: num(q.destination_x), y: num(q.destination_y) });
    }
    out.perJumpXp = out.reached ? Math.round(out.xp / out.reached) : 0;
    return out;
  }
  // Record every waypoint the voyager reaches, once, into the session.
  function trackVoyager() {
    var s = voyagerSummary();
    if (!s) return;
    if (!session.voy || !Array.isArray(session.voy.ids)) session.voy = { ids: [], jumps: 0, xp: 0, dust: 0, net: 0 };
    var changed = false;
    for (var i = 0; i < s.items.length; i++) {
      var it = s.items[i];
      if (!it.reached || session.voy.ids.indexOf(it.id) >= 0) continue;
      session.voy.ids.push(it.id);
      if (session.voy.ids.length > 300) session.voy.ids.splice(0, session.voy.ids.length - 300);
      session.voy.jumps += 1; session.voy.xp += it.xp; session.voy.dust += it.gross; session.voy.net += it.net;
      changed = true;
      log("voyager", { reached: [it.x, it.y], xp: it.xp, dust: it.gross });
    }
  ...
  function render(ex) {
    ...
    lines.push("<b>" + t("Fuel") + "</b> <span style='" + fuelStyle + "'>" + fmt(fi.fuel) + (fi.total ? " / " + fmt(fi.total) : "") + "</span> · " + t("avg jump {n}", { n: fi.avg.toFixed(2) }) +
    ...
    lines.push("<b>" + t("Session") + "</b> " + dur(elapsed) + " · " + t(session.jumps === 1 ? "{n} jump" : "{n} jumps", { n: session.jumps }) + " · " + t("{n} new", { n: session.newSystems }) +
      ...
      " · " + t("XP {n}", { n: fmt(session.xp) }) +
      (session.voy && session.voy.jumps ? " · " + t("voyager {j} jumps, XP {x}, dust {d}", { j: session.voy.jumps, x: fmt(session.voy.xp), d: fmt(session.voy.dust) }) : "") +
      " " + link("reset", t("reset")));
    var voy = voyagerSummary();
    if (voy) {
      var etaMs = voy.eta - Date.now();
      lines.push("<b style='color:#4dd0e1'>" + t("Voyager") + "</b> " + t("{a}/{b} waypoints reached", { a: voy.reached, b: voy.total }) +
        (voy.reached ? " · " + t("dust {n}", { n: fmt(voy.dust) }) + " <span style='opacity:.7'>(" + t("net {n}", { n: fmt(voy.net) }) + ")</span> · " + t("XP {n}", { n: fmt(voy.xp) }) + " <span style='opacity:.7'>(" + t("{n}/jump", { n: fmt(voy.perJumpXp) }) + ")</span>" : "") +
        (voy.pending && etaMs > 0 ? " · " + t("ETA {t}", { t: etaMs < 3600000 ? mmssLeft(etaMs) : dur(etaMs) }) : voy.pending ? "" : " · <span style='color:#69f0ae'>" + t("finished") + "</span>"));
    }
  ...
  function tick(ex) {
    trackRewards(ex);
    trackPosition(ex);
    try { trackVoyager(); } catch (e) { log("error", { where: "voyager", error: String(e) }); }
    learnNodes(ex);
    ensurePanel();
  ...
  return {
    ...
    voyager: voyagerSummary,
    trackVoyager: trackVoyager,
    prefs: prefs,
    centerOn: centerOn,
  };
}
